...
return (
    ...
    <motion.div
        ...
        {/* Header */}
        <div
            className="p-4 sm:p-6 border-b border-slate-500/30 flex-shrink-0 bg-gradient-to-r from-slate-800/50 to-slate-700/30">
            <div
                ...
                <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <h2 className="text-2xl sm:text-3xl font-display font-bold gradient-text">Template Gallery
                                            </h2>
                    </div>
                    <p className="text-slate-400 text-sm sm:text-base">Choose from {templates.length}professionally designed templates to accelerate your goal achievement
                                      </p>
                </div>
                <Button
                    variant="ghost"
        ...
        <div
            className="px-4 sm:px-6 py-4 border-b border-slate-500/30 overflow-x-auto flex-shrink-0 bg-slate-800/20">
            <div className="flex gap-3 min-w-max">
                {categories.map(category => {
                    const categoryCount = category.value === "all" ? templates.length : templates.filter(template => template.category === category.value).length;
                    ...
                    ${isActive ? "bg-gradient-to-r from-primary to-accent text-white shadow-lg shadow-primary/25" : "bg-slate-700/50 text-slate-300 hover:bg-slate-600/50 hover:text-white"}
                  `}
            ...
            {loading ? <div className="space-y-6">
                ...
                <div className="text-center py-8">
                    <div className="relative">
                        <div
                            className="w-16 h-16 mx-auto mb-4 bg-gradient-to-br from-primary/20 to-accent/20 rounded-2xl flex items-center justify-center">
                            <div
                                className="w-8 h-8 border-3 border-primary border-t-transparent rounded-full animate-spin"></div>
                        </div>
                ...
                <div
                    ...
                    {Array.from({
                        ...
                        key={i}
                        className="bg-slate-800/50 rounded-xl p-6 space-y-4 animate-pulse">
                ...
                <div className="text-center max-w-md">
                    <div
                        className="w-16 h-16 mx-auto mb-6 bg-error/20 rounded-2xl flex items-center justify-center">
                        <ApperIcon name="AlertCircle" size={32} className="text-error" />
                    </div>
                    <Error message={error} onRetry={loadData} />
                </div>
            </div> : filteredTemplates.length === 0 ? <div className="text-center py-16">
                <div
                    className="w-20 h-20 mx-auto mb-6 bg-slate-700/50 rounded-2xl flex items-center justify-center">
                    <ApperIcon
                        name={searchQuery ? "Search" : "Package"}
                        size={40}
                ...
                <p className="text-slate-400 mb-6 max-w-md mx-auto">
                    {searchQuery ? "Try adjusting your search terms or browse different categories to find the perfect template." : `We're working on adding more templates to the ${activeCategory} category. Check back soon!`}
                </p>
                {searchQuery && <Button
                    variant="secondary"
                    onClick={() => setSearchQuery("")}
                    className="mx-auto">
                    <ApperIcon name="X" size={16} className="mr-2" />Clear Search
                                    </Button>}
            </div> : <div
                ...
                <AnimatePresence mode="popLayout">
                    {filteredTemplates.map((template, index) => {
                      ...
                      `}
                                ...
                                <div className="relative">
                                    <h3
                                        className="font-semibold text-white mb-2 group-hover:text-primary transition-colors duration-200 line-clamp-1">
                                        {template.title}
                                    </h3>
                                    ...
                                    <div className="flex items-center justify-between pt-2">
                                        ...
                                        <motion.div
                                            ...
                                            <Button
                                                ...
                                                <ApperIcon name="Plus" size={14} className="mr-1" />Use
                                                                            </Button>
                                        </motion.div>
                                    </div>
                                </div>
    ...
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-4 text-sm text-slate-400">
            ...
            {searchQuery && <div
                className="hidden sm:flex items-center gap-2 text-xs bg-slate-700/50 px-3 py-1 rounded-full">
        ...
        <div className="flex items-center gap-3">
            <Button
                ...
                <ApperIcon name="RotateCcw" size={16} className="mr-2" />Reset Filters
                              </Button>
            <Button
                variant="secondary"
                onClick={onClose}
                className="bg-slate-700/50 hover:bg-slate-600/50 border-slate-600/50">
                <ApperIcon name="X" size={16} className="mr-2" />Close Gallery
                              </Button>
        </div>
    </div>
</div>
